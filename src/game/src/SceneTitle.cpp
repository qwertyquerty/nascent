...
namespace nascent {
    ...
    void SceneTitle::init(Game* game) {
        ...
        logo_chart = new Chart(R"(C:\Users\Madeline\Desktop\nascent\assets\songs\logo\logo.json)");

        bg_skin = new Skin("default", 4);
        bg_chart = new Chart(R"(C:\Users\Madeline\Desktop\nascent\assets\songs\title_2\title2.osu.json)");
    ...
    void SceneTitle::update(Game* game, float elapsed_time) {
        ...
        for (uint8_t i = 0; i < SCENE_TITLE_FIELD_COUNT; i++) {
            bg_field[i]->pos.x -= elapsed_time * SCENE_TITLE_SCROLL_RATE_PX_PER_S;
